...
type Slot = { id: string; name: string; sku: string; description: string; requirements: string; imageCount: number; referenceImages: string[]; plannedImages: ProjectImagePlan[]; status: '待配置' | '排队中' | '处理中' | '已完成' | '失败' }
...
export default function BatchProjectWorkspace({ onRunProject }: { onRunProject?: (id: string) => Promise<void> }) {
  ...
  return <section className="mt-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-gray-900">
    ...
    <div className="mt-3 flex flex-wrap gap-2">{slots.map((slot) => <button key={`export-${slot.id}`} type="button" onClick={() => exportProject(slot)} className="rounded-lg border px-3 py-1.5 text-xs text-blue-600">导出 {slot.name} ZIP</button>)}</div>
  </section>
}
