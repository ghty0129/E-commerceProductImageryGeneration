...
type Slot = { id: string; name: string; sku: string; description: string; requirements: string; imageCount: number; status: '待配置' | '排队中' | '处理中' | '已完成' | '失败' }
const initial: Slot[] = Array.from({ length: 5 }, (_, index) => ({ id: `project-${index + 1}`, name: `项目 ${index + 1}`, sku: '', description: '', requirements: '', imageCount: 4, status: '待配置' }))
