...
function normalizeStatus(value: unknown): BatchProjectStatus {
  if (value === 'running' || value === 'submitted' || value === 'partial' || value === 'done' || value === 'error') return value
  if (value === 'queued') return 'partial'
  return 'draft'
}
...
function loadSlots(): BatchProjectSlot[] {
  try {
    ...
    return value.map((slot, index) => ({
      ...
      taskLinks: Array.isArray(slot.taskLinks)
        ? slot.taskLinks
        : Array.isArray(slot.taskIds)
          ? slot.taskIds.flatMap((taskId: unknown, imageIndex: number) => typeof taskId === 'string' && taskId ? [{ imageIndex, taskId }] : [])
          : [],
...
type Props = {
  onRunProject?: (slot: BatchProjectSlot, imageIndexes: number[], onTaskCreated: (taskId: string, imageIndex: number) => void) => Promise<void>
...
export default function BatchProjectWorkspace({ onRunProject, onLoadProject }: Props) {
  ...
  useEffect(() => {
    setSlots((current) => {
      ...
      const next = current.map((slot) => {
        if (slot.taskLinks.length === 0 || slot.status === 'queued') return slot
        const status = deriveBatchProjectStatus(slot.imageCount, slot.taskLinks, tasks)
  ...
  const runProjects = async (ids: string[], retryOnly = false) => {
    if (!ids.length || isQueueRunning) return
    setIsQueueRunning(true)
    setSlots((current) => current.map((slot) => ids.includes(slot.id) ? { ...slot, ...(retryOnly ? {} : { taskLinks: [] }), status: 'queued' } : slot))
    ...
    const results = await runBoundedProjectQueue(snapshot, async (slot) => {
      ...
      const plannedImages = retryOnly && slot.plannedImages.length === slot.imageCount ? slot.plannedImages : Array.from({ length: slot.imageCount }, (_, index) => ({
      ...
      const imageIndexes = retryOnly ? getRetryableImageIndexes(slot.imageCount, slot.taskLinks, tasks) : plannedImages.map((_, index) => index)
      if (onRunProject && imageIndexes.length > 0) {
        await onRunProject({ ...slot, plannedImages, status: 'running' }, imageIndexes, (taskId, imageIndex) => {
          setSlots((current) => current.map((item) => {
            if (item.id !== slot.id) return item
            const taskLinks = [...item.taskLinks.filter((link) => link.imageIndex !== imageIndex), { imageIndex, taskId }]
            return { ...item, taskLinks, status: 'running' }
          }))
        })
      }
    }, 2)
    ...
    <section className="mt-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-gray-900">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <button disabled={isQueueRunning} onClick={() => setSelected(allSelected ? [] : slots.map((slot) => slot.id))} className="rounded-lg border px-3 py-2 text-xs">{allSelected ? '取消全选' : '全选'}</button>
          <button disabled={!selected.length || isQueueRunning} onClick={() => void runProjects(selected)} className="rounded-lg bg-blue-600 px-3 py-2 text-xs text-white disabled:opacity-40">{isQueueRunning ? '队列执行中…' : `执行队列（${selected.length}）`}</button>
        </div>
      </div>
      <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
        {slots.map((slot) => (
          <div key={slot.id} className={`rounded-xl border p-3 ${selected.includes(slot.id) ? 'border-blue-400 bg-blue-50 dark:bg-blue-400/10' : 'border-gray-200 dark:border-white/10'}`}>
            ...
            <label className="mt-1 flex items-center gap-2 text-[11px]">图片数<input type="number" min={1} max={12} value={slot.imageCount} onChange={(event) => patchSlot(slot.id, { imageCount: Math.min(12, Math.max(1, Number(event.target.value) || 1)), plannedImages: [], taskLinks: [], status: 'draft' })} className="w-14 rounded border px-1 py-0.5 dark:bg-gray-950" /></label>
            ...
            <div className="mt-2 text-[11px] text-gray-500">{BATCH_PROJECT_STATUS_LABELS[slot.status]}{slot.taskLinks.length ? ` · ${slot.taskLinks.length} 个任务` : ''}</div>
            {slot.status === 'error' || slot.status === 'partial' ? <button type="button" disabled={isQueueRunning} onClick={() => void runProjects([slot.id], true)} className="mt-2 rounded border border-red-200 px-2 py-1 text-[11px] text-red-600 disabled:opacity-40">继续未完成图片</button> : null}
          </div>
        ))}
      </div>
