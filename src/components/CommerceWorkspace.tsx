...
import type { ProductFactCard } from '../lib/productFacts'
  ...
  return (
    <section className="mt-6 rounded-2xl border border-gray-200 bg-gray-50 p-4 shadow-sm dark:border-white/[0.08] dark:bg-gray-950 sm:p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div><h2 className="text-lg font-bold text-gray-900 dark:text-white">{mode === 'free' ? '自由创作工作台' : '平台通用工作台'}</h2><p className="mt-1 text-xs text-gray-500">本阶段建立独立草稿与规则边界；结构化提示词和逐张图片方案将在后续阶段接入。</p></div>
        ...
        <div className="space-y-4">
          ...
          {mode === 'universal' ? (
            <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-white/[0.08] dark:bg-gray-900">
              ...
              <label className="mt-3 block"><span className="mb-1.5 block text-xs font-semibold text-gray-600 dark:text-gray-300">整套要求</span><textarea value={workspace.universal.globalRequirements} onChange={(event) => setWorkspace((current) => ({ ...current, universal: { ...current.universal, globalRequirements: event.target.value } }))} className={`${FIELD_CLASS} min-h-28 resize-y`} placeholder="例：整套保持简洁科技风，商品外观必须一致，可加入自有品牌Logo。" /></label>
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-white/[0.08] dark:bg-gray-900">
              ...
              <label className="mt-3 block"><span className="mb-1.5 block text-xs font-semibold text-gray-600 dark:text-gray-300">整套创作要求</span><textarea value={workspace.free.globalRequirements} onChange={(event) => setWorkspace((current) => ({ ...current, free: { ...current.free, globalRequirements: event.target.value } }))} className={`${FIELD_CLASS} min-h-32 resize-y`} placeholder="例：生成4张，右下角添加半透明自有品牌水印；两张户外场景、一张细节、一张竖版海报。" /></label>
              <div className="mt-3 flex flex-wrap gap-3 text-sm text-gray-700 dark:text-gray-200">
                {([['allowText', '允许图片文字'], ['allowOwnedLogo', '允许自有Logo'], ['allowWatermark', '允许自有水印']] as const).map(([key, label]) => <label key={key} className="inline-flex items-center gap-2 rounded-lg bg-gray-50 px-3 py-2 dark:bg-white/[0.05]"><input type="checkbox" checked={workspace.free[key]} onChange={(event) => setWorkspace((current) => ({ ...current, free: { ...current.free, [key]: event.target.checked } }))} />{label}</label>)}
              </div>
            </div>
          )}
        </div>
...
export default function CommerceWorkspace() {
  ...
  const profileError = profile ? validateApiProfile(profile) : '未选择支持Chat Completions或Responses API的AI策划配置'
